...
Next, measured and waiting (2026-09-10): `assignStress`'s prefix rules put the primary on the wrong syllable for property/process/proper/expert/reference/conference/advertise (they return index 1, the dict wants 0) and for yesterday/percent/internet — no vowel rule can reach those. The same is true of most of the remaining `a` errors: of 1475 æ→ə positions, 788 sit where the rules stress syllable 0 and the dict reduces it (abet, abort, abash), and 1519 of the 1949 ə→æ positions are dict-stressed. `VOWEL_DIGRAPHS` has no `oi` (avoid) and `DIGRAPH_RIME` does not treat a magic-e or r-controlled rime as tense (alone, alert), which is where the weak `a-` prefix rule loses them.
