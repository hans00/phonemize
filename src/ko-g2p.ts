...
class KoreanG2P implements LanguageProcessor {
  ...
  private decomposeRomajaSyllable(syllable: string): KoreanSyllable {
    ...
    const vowelKeys = Object.keys(KOREAN_VOWELS).sort(
    ...
    if (!medial) {
      // If no vowel found, it's not a valid syllable
  ...
  private processKorean(text: string): string {
    ...
    const decomposed: KoreanSyllable[] = syllables.map((s) =>
      this.decomposeRomajaSyllable(s),
    );
    ...
    for (let i = 1; i < decomposed.length; i++) {
      ...
      if (
        curr.initial === "h" &&
        ["g", "k", "d", "t", "b", "p", "j", "ch"].includes(prev.final)
      ) {
      ...
      if (
        ["n", "m"].includes(curr.initial) &&
        ["g", "k", "d", "t", "b", "p", "s", "ss", "j", "ch"].includes(
          prev.final,
        )
      ) {
    ...
    for (let i = 0; i < decomposed.length; i++) {
      ...
      if (i > 0 && ["k", "t", "p", "tɕ"].includes(initialIpa)) {
        ...
        if (
          KOREAN_VOWELS[prev.medial] ||
          ["n", "m", "ŋ", "l"].includes(prevFinalIpa)
        ) {
          const voiced: { [key: string]: string } = {
            k: "ɡ",
            t: "d",
            p: "b",
            tɕ: "dʑ",
