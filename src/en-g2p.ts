...
export class EnglishG2P implements LanguageProcessor {
  ...
  private predictInternal(
    ...
    if (syllableIPA.length > 0) {
      ...
      result = result.replace(/ɹɪtʃ$/, 'ɹɪk');  result = result.replace(/ɡdʒ$/, 'ɡ');  result = result.replace(/(?<=[aɑɔɛɪouəɝ])dʒɝ$/, 'ɡɝ');  result = result.replace(/ətʃ$/, 'ək');  // Germanic -rich/-gge; vowel+ger → hard g (eager/tiger/meager); -ach/-ech/-och → k (monarch/epoch/stomach)
