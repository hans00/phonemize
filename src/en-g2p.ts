...
export class EnglishG2P implements LanguageProcessor {
  ...
  private predictInternal(
    ...
    if (syllableIPA.length > 0) {
      ...
      result = result.replace(/([kɡdbptfvszʃʒmnlɹwj])əəs$/g, '$1uəs');  result = result.replace(/ksɪəs/g, 'kʃəs');  result = result.replace(/stjʊɹ/g, 'stʃɝ');  // -uous → uəs; -xious → kʃəs; -sture → stʃɝ (gesture/pasture)
