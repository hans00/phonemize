...
export class EnglishG2P implements LanguageProcessor {
  ...
  private predictInternal(
    ...
    if (syllableIPA.length > 0) {
      ...
      result = result.replace(/oʊɹ/g, 'ɔɹ');  result = result.replace(/[ɑə][ɛə]$/, 'oʊ');  result = result.replace(/oʊ([ntplm])ɪk/g, 'ɑ$1ɪk');  result = result.replace(/oʊnəm/g, 'ɑnəm');  result = result.replace(/oʊmɪtɝ/g, 'ɑmɪtɝ');  result = result.replace(/oʊɡɹəf([iɝ])/g, 'ɑɡɹəf$1');  result = result.replace(/oʊmɪnən([ts])/g, 'ɑmənən$1');  result = result.replace(/oʊdʒɪk$/, 'ɑdʒɪk');  result = result.replace(/ənoʊ/g, 'ɑnoʊ');  result = result.replace(/ætɪv$/, 'ətɪv');  result = result.replace(/nɑɹi$/, 'nɛɹi');  result = result.replace(/ɑɹi$/, 'ɛɹi');  if (lowerWord.endsWith('erate') || lowerWord.length >= 9) result = result.replace(/[ɛɔ]ɹeɪt(ɝ?)$/, 'ɝeɪt$1');  result = result.replace(/æbli$/, 'əbli');  // o-open-syllable before r; -oe; -onic/-opic/-olic/-omic/-otic; -onomy; -ometer; -ography/-ographer; -minant/-minance; -logic; -ano; -ative; -inary/-enary; -ary/-ory; -ably
      ...
      result = result.replace(/([kɡdbptfvszʃʒmnlɹwj])əəs$/g, '$1uəs');  result = result.replace(/ksɪəs/g, 'kʃəs');  result = result.replace(/stjʊɹ/g, 'stʃɝ');  result = result.replace(/([tsn])aɪv$/g, '$1ɪv');  if (lowerWord.includes('asiv')) result = result.replace(/æsɪv/, 'eɪsɪv');  result = result.replace(/([^aeiouæɑɔɛɪuoəɝʌ])ɪtɪv$/g, '$1ətɪv');  result = result.replace(/ɡuʃ/g, 'ɡwɪʃ');  result = result.replace(/məkɝ$/, 'meɪkɝ');  result = result.replace(/məkɪŋ$/, 'meɪkɪŋ');  result = result.replace(/məstɝ$/, 'mæstɝ');  if (lowerWord.endsWith('ator')) result = result.replace(/([^w])ətɝ$/, '$1eɪtɝ');  if (lowerWord.endsWith('mony') && syllables.length >= 3) result = result.replace(/məni$/, 'moʊni');  result = result.replace(/ɪ([nlkfvmt])eɪʃən$/, 'ə$1eɪʃən');  result = result.replace(/ɪ([nk])eɪt$/, 'ə$1eɪt');  result = result.replace(/(s|dʒ)ɪbəl$/g, '$1əbəl');  if (!lowerWord.endsWith('sense') && !lowerWord.endsWith('fense')) result = result.replace(/([ɪɛ])ns$/, 'əns');  if (lowerWord.endsWith('inger')) result = result.replace(/ndʒɝ$/, 'ŋɝ');  if (lowerWord.endsWith('unger') || lowerWord.endsWith('onger')) result = result.replace(/ndʒɝ$/, 'ŋɡɝ');  result = result.replace(/ʌmɪnəs$/, 'umənəs');  result = result.replace(/pənənt$/, 'poʊnənt');  result = result.replace(/ɡɹɛɡeɪt$/, 'ɡɹəɡeɪt');  result = result.replace(/tɪənɛɹi$/, 'ʃənɛɹi');  if (!lowerWord.endsWith('cission')) result = result.replace(/sɪʃən$/, 'zɪʃən');  result = result.replace(/ɝɹeɪʃən$/, 'ɝeɪʃən');  if (lowerWord.length >= 9) result = result.replace(/ɪɹeɪʃən$/, 'ɝeɪʃən');  result = result.replace(/ɪnɪti$/, 'ɪnəti');  result = result.replace(/ɛntɛɹi$/, 'əntɛɹi');  result = result.replace(/ɪbli$/, 'əbli');  result = result.replace(/ɪtud$/, 'ətud');  if (lowerWord.endsWith('iment')) result = result.replace(/ɪmənt$/, 'əmənt');  if (lowerWord.endsWith('ancy')) result = result.replace(/ænsi$/, 'ənsi');  result = result.replace(/oʊleɪt$/, 'ɑleɪt');  if (lowerWord.endsWith('erage') || lowerWord.endsWith('erature')) result = result.replace(/ɛɹ/g, 'ɝ');  // -uous/-xious/-sture/-itive/-asive/-guish; -maker/-master; -ator; -mony; -ination/-icate/-inate; -ence/-ance; -inger/-unger/-onger; -uminous; -ponent; -gregate; -tionary; -sition; -iration; -inity; -entary; -ibly; -itude; -iment; -ancy; -olate; -erage/-erature
  ...
  private tryMorphologicalAnalysis(word: string): string | undefined {
    ...
    if (lowerWord.endsWith('ual') && !lowerWord.endsWith('gual') && lowerWord.length > 5 && !(lowerWord.endsWith('tual') && lowerWord.length > 6)) { const b = lowerWord.slice(0, -3), p = this.wellKnown(b, undefined, true) || this.predictInternal(b, undefined, false); if (p) return p.replace(/[uʊ]$/, '') + 'uəl'; }
  ...
  private syllableToIPA(syllable: string, syllableIndex: number, isStressed: boolean, isLastSyllable: boolean, nextSyllable?: string, steps?: TraceStep[], prevSyllable?: string, isNextLastSyllable = false): string {
    ...
    if (!isStressed && isLastSyllable && syllableIndex > 0 && !/all$/i.test(syllable)) {
      applyReduction({
        'ɑɹ': 'ɑɹ', 'ɔɹ': 'ɔɹ', 'ɔɪ': 'ɔɪ',
        'æ': 'ə', 'ɛ': 'ə', 'ɑ': 'ə', 'ʌ': 'ə', 'ɔ': 'ə',
      });
