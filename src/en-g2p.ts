...
export class EnglishG2P implements LanguageProcessor {
  ...
  private predictInternal(
    ...
    if (syllableIPA.length > 0) {
      ...
      result = result.replace(/ɹɪtʃ$/, 'ɹɪk');  result = result.replace(/ɡdʒ$/, 'ɡ');  result = result.replace(/(?<=[aɑɔɛɪouəɝ])dʒɝ$/, 'ɡɝ');  result = result.replace(/ətʃ$/, 'ək');  result = result.replace(/([bdfɡhklmnpɹstzv])ə(ʃ|dʒ)əs$/, '$1eɪ$2əs');  result = result.replace(/([^w])əʃən$/, '$1eɪʃən');  if (/asi(ve|on|vely)$/.test(lowerWord)) result = result.replace(/æ(s[ɪi])/, 'eɪ$1');  // Germanic -rich/-gge; vowel+ger; -ach → k; -acious; -ation; -asive (abrasive/invasive)
