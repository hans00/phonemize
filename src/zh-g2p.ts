...
class ChineseG2P implements LanguageProcessor {
    ...
    try {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        if (this.isChinese(char)) {
          const pinyinResult =
            pinyinResults[Math.min(i, pinyinResults.length - 1)];
          const { syllable, tone } = this.parsePinyinWithTone(
            pinyinResult || char,
          );
   ...
   */
  private parsePinyinWithTone(pinyinWithTone: string): {
    syllable: string;
    tone: number;
  } {
